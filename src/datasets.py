...
class UFC101Dataset(Dataset):
    def __init__(self, directory, file_ind, trainlist, transform=None, num_frames=None, num_samples_use=50, classes_to_use=None, mode='train'):
        """
        ...
        self.class_indixes = self.load_class_indices(file_ind)
        self.video_paths, self.labels = self.load_video_list(trainlist, mode)
...
if __name__ == '__main__':
    ...
    ucf_dataset = UFC101Dataset(video_dir, file_ind=file_ind_path, trainlist=train_list_path, transform=transform, num_frames=16, classes_to_use=classes_to_use, mode='train')
    batch_size = 4
